...
public class Mfrg002Controller extends AbstractAhisController<Mfrg002Form> {
	...
	public String index(@ModelAttribute Mfrg002Form mfrg002Form) {
		...
		RegisterBaseHisRequest request = new RegisterBaseHisRequest();
		request.setBeginDate(year + "-" + month + "-" + day);
		request.setEndDate(year + "-" + month + "-" + day);
		request.setDeptCode(mfrg002Form.getDeptCode());

		List<RegisterBaseItem> regInfoList = mfrg002Service.getRegisterInfoList(request);
			...
			for (int i = 0; i < regInfoList.size(); i++) {
				RegisterBaseItem dbItem = regInfoList.get(i);
